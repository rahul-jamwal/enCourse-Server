...
export const login = catchAsyncError(async (req, res, next) => {
    ...
    if (!user) return next(new ErrorHandler("User doesn't exists", 409))
...
export const forgotPassword = catchAsyncError(async (req, res, next) => {
    ...
    const message = `Click on the link to reset your password. ${url}. If yoou have not request then please ignore.`

    //send token via email
    await sendEmail(user.email, "enCourse Reset Password", message)
...
export const resetPassword = catchAsyncError(async (req, res, next) => {
    ...
    if(!user) return next(new ErrorHandler("Token invalid or has been expired"))
...
User.watch().on("change",async ()=>{
    ...
    const subscription = await User.find({"subscription.id" : "active"})
